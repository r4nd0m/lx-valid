(function (exports) {
    ...
    function validateObject (object, schema, options, errors) {
        var props, p, pprops, isPatternOk, ppRegEx, allProps = Object.keys(object),
            visitedProps = [];
        ...
        if (options.unknownProperties) {
            ...
            for (p in object) {
                if (object.hasOwnProperty(p)) {
                    if (props.indexOf(p) === -1) {
                        // check if property name conforms pattern properties, hence NOT "unknown"
                        var isPatternOk = false;
                        if(schema.patternProperties){
                            for(var pprops in schema.patternProperties){
                                if (schema.patternProperties.hasOwnProperty(pprops)) {
                                    ppRegEx = new RegExp(pprops);
                                    if(ppRegEx.test(p)){
                                        isPatternOk = true;
                                        break;
                                    }
